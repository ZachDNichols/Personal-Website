...
if (names.length > 0) {
    for (let i = 0; i < names.length; i++) {
        ...
        <div class='github-block'> \
            <a href='";
        if (html_urls[i]["html_url"] != null && html_urls[i]["html_url"] != "undefined") {
            html += html_urls[i]["html_url"];
        }
        html += "'>";
        html += "<div class='github-header'><h2>";
        if (names[i]["name"] != null && names[i]["name"] != "undefined") {
            html += names[i]["name"];
        }
        html += "</h2> \
            <svg \
            ...
            <h3>";
        if (full_names[i]["full_name"] != null && full_names[i]["full_name"] != "undefined") {
            html += full_names[i]["full_name"];
        }
        if (languages[i]["language"] != null && languages[i]["language"] != "undefined") {
            html += " - ";
            html += languages[i]["language"];
        }
        html += "</h3> \
            <p>";
        if (descriptions[i]["description"] != null && descriptions[i]["description"] != "undefined") {
            html += descriptions[i]["description"];
        }
        html += "</p> \
            <p> Stars: ";
        if (stargazers_counts[i]["stargazers_count"] != null && stargazers_counts[i]["stargazers_count"] != "undefined") {
            html += stargazers_counts[i]["stargazers_count"];
        }
        html += " Watching: ";
        if (watchers[i]["watchers"] != null && watchers[i]["watchers"] != "undefinded") {
...
{
    html += "<p> Failed to load projects.</p>";
}
